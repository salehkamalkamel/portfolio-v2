...
export default function SignUpPage() {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    setIsLoading(true);
    try {
      // Using authClient.signUp.email for registration
      const { error } = await authClient.signUp.email({
        email: data.email,
        password: data.password,
        name: data.name,
        callbackURL: "/blog", // Redirect after successful signup
      });
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-neutral-900/50 border border-neutral-800/40 rounded-lg p-8">
          ...
          <p className="text-neutral-400 mb-8">
            Don't worry, I won't spam you. (Okay, maybe a tiny bit if I build
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor="form-rhf-signup-name">Name</FieldLabel>
                    <Input
                      disabled={isLoading}
                      {...field}
