...
import { updatePost } from "@/server/blog-actions"; // Assuming this action exists
import { toast } from "sonner";
import { Author, Post } from "@/db/schema";
...
interface EditBlogFormProps {
  authors: Author[];
  existingPost: Post; // Assuming tags might be joined or array depending on DB
  postId: string;
}
...
export default function EditBlogForm({
  ...
  // Initialize tags from existing post
  ...
  const form = useForm<PostFormValues>({
    ...
    defaultValues: {
      ...
      category: existingPost.category || "Performance",
      // Handle ID as string for the Select component
      authorId:
        existingPost.authorId?.toString() || authors[0]?.id.toString() || "",
      ...
      readTime: existingPost.readTime || 3,
  ...
  function onSubmit(data: PostFormValues) {
    startTransition(async () => {
      // Pass the ID, the form data, and the tags
      const response = await updatePost(postId, { ...data, tags });

      if (!response?.success) {
        toast.error("Failed to update post. Please check the form.");
        // Optional: Throw error to prevent redirect if you want to stay on page
      } else {
          ...
          <div className="space-y-6">
            ...
            <FieldGroup className="space-y-6">
              ...
              {/* <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <FieldLabel className="text-white">Slug</FieldLabel>
                    <FieldDescription className="text-neutral-500 text-sm mb-2">
                      URL-friendly ID. Changing this will break existing links.
                    </FieldDescription>
                    <Input
                      {...field}
                      className="bg-neutral-800/50 border-neutral-700/50 text-white placeholder-neutral-500 focus:border-emerald-500"
